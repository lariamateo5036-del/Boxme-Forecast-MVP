/**
 * Generate 24 months of fake historical data for MVP testing
...
// Main function
async function main() {
  console.log('🚀 Generating 24 months of fake data...\n');

  // Get peak events from seed.sql
  const peakEvents = [
    // 2024
  ...
  while (currentDate <= endDate) {
    ...
    const dayOrders = generateOrdersForDay(currentDate, peakEvent, orderId);
    allOrders.push(...dayOrders);

    totalOrders += dayOrders.length;
    orderId += dayOrders.length;
    totalDays++;

    if (totalDays % 30 === 0) {
      console.log(`  → Processed ${totalDays} days, ${totalOrders} orders...`);
    }

    currentDate = addDays(currentDate, 1);
  }

  console.log(`\n✓ Generated ${totalOrders} orders over ${totalDays} days`);
  console.log(`Average: ${Math.round(totalOrders / totalDays)} orders/day\n`);
  ...
  const batchSize = 500;
  let sqlContent = '-- Generated fake orders data\n-- Total orders: ' + totalOrders + '\n\n';

  for (let i = 0; i < allOrders.length; i += batchSize) {
    const batch = allOrders.slice(i, i + batchSize);
    sqlContent += 'INSERT OR IGNORE INTO orders_history (\n';
    sqlContent += '  id, order_id, order_date, order_time, fulfillment_center_id, customer_id,\n';
    sqlContent += '  marketplace, product_group, sku_complexity, service_level, shipping_carrier,\n';
    sqlContent += '  num_items, is_delayed_allowed, sla_deadline, completion_time\n';
    sqlContent += ') VALUES\n';
  ...
  // Write to file using Deno APIs
  try {
    await Deno.writeTextFile(sqlFile, sqlContent);
